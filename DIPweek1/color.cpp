...
cv::Mat ColorTransform( cv::Mat img,
                        std::function<cv::Vec3b( cv::Vec3b )> func )
{
    ...
    int i, j;
    cv::Vec3b *p, *q;
    cv::Mat returnImg = cv::Mat::zeros( img.size(), img.type() );
...
{
    double theta = color[0] == color[1] && color[0] == color[2]
                       ? 0.0
                       : std::acos( ( 2 * (double)color[2] - (double)color[1] -
                                      (double)color[0] ) /
                                    2.0 /
                                    std::sqrt( ( color[2] - color[1] ) *
                                                   ( color[2] - color[1] ) +
                                               ( color[2] - color[0] ) *
                                                   ( color[1] - color[0] ) ) );
    double H =
        color[0] <= color[1] ? theta / ( 2 * M_PI ) : 1 - theta / ( 2 * M_PI );
    double S =
    ...
    return cv::Vec3b( threshold( H * 256 ), threshold( S * 256 ),
                      threshold( I ) );
}
